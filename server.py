...
class ServerNode(threading.Thread):
    ...
    def service_callback(self,ch, method, properties, body):
        ...
        if msg_type=="JOIN":
            ...
            self.write_log("CLIENT" + str(assigned_id)+ "registered in group " + str(group))
            ...
            if self.get_server(int_id) == int(self.id):
                ...
                if msg_type =="LEAVE":
                    ...
                    self.write_log("PUBLISHED MESSAGES FOR CLIENT "+str(int_id))
                ...
                if server_id not in self.reconstructed:                    
                    ...
                    if next_server == server_id:
                        ...
                        self.write_log("CLIENT"+str(id)+" has been reregistered on server "+self.id)
                    #server node dead is an intermediary node
                    else:
                        with pub_loc:
                            ch.basic_publish(exchange='',routing_key =self.send_queue ,body=orig_msg,properties=pika.BasicProperties(content_type='text/plain',delivery_mode=1))
                        self.write_log("CLIENT"+str(id)+" is not register on my server, rerouting")
    ...
    def write_log(self, message):
        print("[INFO][SERVER"+str(self.id)+"]", message)
            ...
            if msg_type =="LEAVE":                      
                ...
                for mess in to_send: 
                    ...
                    with pub_loc:
                        ch.basic_publish(exchange=self.exchange,routing_key=self.get_routing(group),body=mess["message"])
                        self.update_message_sent(group,int_id, mess["mid"])
                write_log("PUBLISHED MESSAGES FOR CLIENT "+str(int_id))
        else:
            server_id = self.get_server(int_id)
            if self.id == self.num:
                next_server = 1
                ...
                next_server = self.id + 1 
            write_log("server id ", next_server, self.reconstructed, self.id)
            if server_id not in self.reconstructed:                
                with pub_loc:
                    ch.basic_publish(exchange='',routing_key =self.send_queue ,body=orig_msg)
            else:
    ...
    while True:
        ...
        if "KILLNODE".lower() in msg:
            ...
            for nodes in server_nodes:
                ...
                a=nodes.get_id()
                if a!= None:
                    active_ones.append(a)
            print(active_ones)
            for i, elem in enumerate(active_ones):
                print(elem, "===>", end=" ")
            print(active_ones[0])
            print("\n==============================================")
